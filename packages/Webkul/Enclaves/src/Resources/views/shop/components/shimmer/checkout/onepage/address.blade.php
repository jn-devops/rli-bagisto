<div class="mt-[30px]">
    <div class="flex justify-between items-center py-[10px]">
        <h2 class="shimmer w-[193px] h-[39px]"></h2>
        
        <span class="shimmer w-[24px] h-[24px]"></span>
    </div>

    <div>
        <div
            class="grid gap-[20px] grid-cols-2 mt-[30px] max-1060:grid-cols-[1fr] max-lg:grid-cols-2 max-lg:grid-cols-1 max-lg:mt-[15px]">
            {{-- Single card addredd --}}
            <div class="max-w-[414px] p-[20px] border border-[#e5e5e5] rounded-[12px] max-lg:flex-wrap">
                <div class="flex gap-[10px] justify-between items-center">
                    <p class="shimmer w-[150px] h-[24px]"></p>

                    <div class="flex gap-[25px] items-center">
                        <div class="shimmer block w-[24px] h-[24px] m-0 ml-[0px] rounded-[10px]"></div>
                    </div>
                </div>

                <p class="shimmer w-[100%] h-[21px] mt-[25px]"></p>
            </div>

            {{-- Single card addredd --}}
            <div
                class="flex justify-center items-center max-w-[414px] p-[20px] border border-[#e5e5e5] rounded-[12px] max-lg:flex-wrap">
                <div class="flex gap-x-[10px] items-center">
                    <span class="shimmer w-[52px] h-[52px] rounded-full"></span>

                    <p class="shimmer w-[140px] h-[24px]"></p>
                </div>
            </div>
        </div>
    </div>

    <p class="shimmer w-[40%] h-[21px] mt-[20px]"></p>
</div>

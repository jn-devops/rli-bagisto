    ...
    >
                    ...
                    <x-slot:content class="!p-[0px]">
                        @foreach ($channels as $channel)
                            <a
                                ...
                                class="flex cursor-pointer gap-[10px] px-5 py-2 text-[16px] hover:bg-gray-100 dark:text-white dark:hover:bg-gray-950"
                            >
                                {{ $channel->name }}
                            </a>
                        @endforeach
        ...
        <div class="mt-[14px] flex gap-[10px] max-xl:flex-wrap">
            @foreach ($product->attribute_family->attribute_groups->groupBy('column') as $column => $groups)
                ...
                <div
                    @if ($column == 1) class="flex flex-1 flex-col gap-[8px] max-xl:flex-auto" @endif
                    @if ($column == 2) class="flex w-[360px] max-w-full flex-col gap-[8px] max-sm:w-full" @endif
                >
                    @foreach ($groups as $group)
                        @php
                            if (
                                ! core()->getConfigData('google_feed.settings.general.package_status')
                                && $group->name == "Google Shopping"
                            ) {
                                continue;
                            }
                        ...
                        @if (count($customAttributes))
                            ...
                            <div class="box-shadow relative rounded-[4px] bg-white p-[16px] dark:bg-gray-900">
